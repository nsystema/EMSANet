# This script is a modified version of the EMSANET_RT.py script provided by the EMSANet authors.
import os
import traceback
import cv2
import numpy as np
...
from pointcloud import create_point_cloud_rs, filter_invalid_points
...
OCCUPANCY_GRID_MAPPING = True  # If True, the occupancy grid mapping will be performed
# Grid parameters
GRID_HEIGHT = 3.01        # Grid height in meters
GRID_WIDTH = 3.01         # Grid width in meters
CELL_SIZE = 0.01          # Size of each cell in meters (1 cm)
NUM_POINTS_THRESHOLD = 10 # Minimum number of points to consider a cell as valid
...
def occupany_grid_mapping(predictions, color_frame, depth_frame):
    """
    Perform occupancy grid mapping using the semantic segmentation predictions and the RealSense frames.
    ...
    point_cloud, _ = create_point_cloud_rs(depth_frame, color_frame)
    # create the mask for the floor, floor mat and ceiling
    mask = anything_mask(predictions, 'floor floor_mat ceiling')
    mask = mask.cpu().flatten()
    # apply the mask to the point cloud
    point_cloud = point_cloud[~mask]
    # initialize the occupancy grid
    occupancy_grid = OccupancyGrid(GRID_WIDTH,GRID_HEIGHT, CELL_SIZE, NUM_POINTS_THRESHOLD)
    # process the point cloud
    occupancy_grid.process_pointcloud(point_cloud)
    # visualize the occupancy grid
    return occupancy_grid.get_grid_as_rgb()

def visualize_occupancy_grid(occupancy_grid, window_name='Occupancy Grid'):
    """
    Visualize the occupancy grid mapping results.

    Args:
        occupancy_grid (np.ndarray): The occupancy grid mapping results.
        window_name (str): The name of the display window.
    """
    occupancy_grid = cv2.cvtColor(occupancy_grid, cv2.COLOR_RGB2BGR)
    cv2.imshow(window_name, occupancy_grid)
...
def main():
    ...
    try:
        for depth_frame, color_frame in pipeline:
            ...
            # Perform occupancy grid mapping
            if OCCUPANCY_GRID_MAPPING:
                occupancy_grid = occupany_grid_mapping(predictions, color_frame, depth_frame)
        ...
        # show the function that caused the error
        traceback.print_exc()
    finally:
